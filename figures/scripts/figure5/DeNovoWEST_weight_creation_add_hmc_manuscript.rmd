...
This is a code used to generate weights in the upgraded DeNovoWEST. 

This code was adapted from the scripts published by Joanna Kaplanis (@queenjobo) and Kaitlin Samocha (@ksamocha) : https://github.com/HurlesGroupSanger/DeNovoWEST/blob/master/weight_creation/DeNovoWEST_weight_creation.Rmd
...
rates_file = '/rds/general/user/xzhang13/home/pfam/analysis_human_pfam/4_DNV/dnv_rates/all_rates_withHRC_20211130.txt.gz'
dnm_file_wweights = '/rds/general/user/xzhang13/home/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/input/DDD_RUMC_GDX_denovos_cadd_shet_wweights_2020_01_17.txt.gz'
```

# update the constrained information for observed DNM
```{r}
...
# the file with all hmc constrained missense variants
hrc_constrained_file<-'/rds/general/user/xzhang13/home/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/input/hrc_constrained_missense.txt'
hrc_constrained_snv <- fread(hrc_constrained_file, stringsAsFactors = F, sep = '\t', header = T)

all_wweights<-merge(all_wweights,hrc_constrained_snv,by=c("chrom","pos","ref","alt"),all.x = TRUE)
all_wweights$hrc_constrained=FALSE
all_wweights$hrc_constrained<-ifelse(!is.na(all_wweights$hrc)&(all_wweights$hrc<0.8),TRUE,FALSE)
table(all_wweights$hrc_constrained)
```
...
## For the updated test in HMC, we changed here: 

```{r}
table(all_wweights$constrained)
#FALSE  TRUE 
#41636  3585 
str(all_wweights)
#all_wweights$constrained<-as.logical(toupper(all_wweights$constrained))

rates$constrained<-as.logical(toupper(rates$constrained))
#str(all_wweights)
#str(rates)
all_wweights$constrained<-ifelse(all_wweights$constrained|all_wweights$hrc_constrained,TRUE,FALSE)
all_wweights$constrained<-as.character(all_wweights$constrained)
table(all_wweights$constrained)
all_wweights$constrained<-ifelse(all_wweights$constrained=="TRUE","True","False")
table(all_wweights$constrained)
table(all_wweights$constrained)
#False  True 
#41036  4201 
...
str(rates)
table(rates$constrained)
#    FALSE      TRUE 
#107443080   3971226 
rates$constrained<-ifelse(rates$constrained|rates$hrc_constrained,"True","False")

table(rates$constrained)
#    False      True 
#106710676   4703630

fwrite(rates, "/rds/general/user/xzhang13/home/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/update/all_rates_update_20211130.txt.gz",quote=F,sep="\t",compress="gzip")
fwrite(all_wweights, "/rds/general/user/xzhang13/home/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/update/all_dnv_update_31KDD.txt",quote=F,sep="\t")
...
# recreate the weights for Missense variants 
...
## save data to compare the change of weights with the original paper
new_mis_oe_bin=obs_exp_shet2bins_cadd6bins_missense
save(new_mis_oe_bin,file="~/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/weight_creation/new_mis_oe_bin.RData")
```


### there are very little counts for HRC constrained - maybe I could add in as either
...
```{r}
write.table(ppv,"~/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/update/ppv_2021_1130.txt",sep="\t",quote=F,col.names = T,row.names = F)

```

```{r make ppv plots, eval=FALSE, include=FALSE}
library(tidyr)
library(cowplot)
...
write.table(reorder_ppv, file = "~/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/update/update_weights_ppv_20211130.tab", sep = "\t", row.names = F, quote = F)
```


```{r}
mis_weights<-subset(reorder_ppv,cq=="missense")
non_weights<-subset(reorder_ppv,cq!="missense")

non_weights$shethigh<-0
non_weights$shetlow<-0

mis_weights_file<-rbind(mis_weights,non_weights)
mis_weights_file$constrained<-ifelse(mis_weights_file$constrained=="True","True","False")
mis_weights_file<-mis_weights_file[as.character(1:nrow(mis_weights_file)),]
write.table(mis_weights_file,file="~/pfam/analysis_human_pfam/4_DNV/DeNovoWEST/update/update_weights_missense_20211130.tab",sep="\t",col.names = T,row.names = F,quote=F)
